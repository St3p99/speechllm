            ...
            if "share memory" in str(e).lower() or "tied" in str(e).lower():
                ...
                try:
                    ...
                    if self.model and hasattr(self.model, "config"):
                        ...
                        self.model.config.save_pretrained(output_dir)
                    logger.info(
                        "Successfully saved model with safe_serialization=False"
                    )
                finally:
...
def train(path_to_yaml_config: Union[str, Path]):
    ...
    if isinstance(training_args.weight_decay, str):
        training_args.weight_decay = float(training_args.weight_decay)
    ...
    model = SpeechLLM(config=SpeechLLMConfig(**model_configs))
